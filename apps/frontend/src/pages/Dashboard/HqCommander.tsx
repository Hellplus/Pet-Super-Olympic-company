...
const HqCommanderDashboard: React.FC = () => {
  ...
  const sponsorChartData = sponsorRanking.slice(0, 10).map((item: any, idx: number) => ({
    name: `分会${idx + 1}`, value: Number(item.totalAmount || 0),
  }));
    ...
    <div>
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} lg={6}>
          <Card><Statistic title="应收账款总额" value={Number(receivable.totalReceivable || 0)} precision={2} prefix={<DollarOutlined />} suffix="元" valueStyle={{ color: '#cf1322' }} /></Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card><Statistic title="已收款" value={Number(receivable.paidAmount || 0)} precision={2} prefix={<CheckCircleOutlined />} suffix="元" valueStyle={{ color: '#3f8600' }} /></Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card><Statistic title="待收款" value={Number(receivable.unpaidAmount || 0)} precision={2} prefix={<AlertOutlined />} suffix="元" valueStyle={{ color: '#faad14' }} /></Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card><Statistic title="智库活跃专家" value={Number(expertStats.activeExperts || 0)} prefix={<TeamOutlined />} suffix={`/ ${expertStats.totalAssignments || 0} 次派遣`} /></Card>
        </Col>
      </Row>
